...
struct Cuboid {
  ...
  void for_each_point(Consumer consumer) const {
    for (Unit x = min.x; x < max.x; ++x) {
      for (Unit y = min.y; y < max.y; ++y) {
        for (Unit z = min.z; z < max.z; ++z) {
          consumer(Point3D<Unit>{ x, y, z });
        }
      }
    }
  }
...
struct VirtualAxis {
  ...
  std::optional<std::pair<size_t, size_t>> find_virtual_segment(int min, int max) const {
    size_t it = 0;
...
struct Universe {
  ...
  [[nodiscard]] std::optional<Cuboid<size_t>> transform(Cuboid<int> cube) const {
    ...
    if (xs && ys && zs) {
      return Cuboid<size_t> {
        Point3D<size_t> { xs->first, ys->first, zs->first },
        Point3D<size_t> { xs->second, ys->second, zs->second }
      };
    } else {
      return std::nullopt;
    }
  ...
  void travel_the_universe(Consumer consumer) const {
    const Cuboid<size_t> whole_area {
      Point3D<size_t>{ 0, 0, 0 },
      Point3D<size_t>{ x.size(), y.size(), z.size() }
    };

    whole_area.for_each_point(consumer);
  }

  [[nodiscard]] long long int operator[](Point3D<size_t> point) const {
...
struct HitTheLights {
  ...
  void run(const std::vector<Instruction> & instructions) {
    for (Instruction instruction : instructions) {
      ...
      new_cube->for_each_point([&](Point3D<size_t> point) {
        blackstar[to_1D_position(point)] = instruction.add;
      });
    }
  }

  [[nodiscard]] size_t to_1D_position(Point3D<size_t> point) const {
    return point.x
  ...
  [[nodiscard]] long long int the_answer_is() const {
    ...
    universe.travel_the_universe([&](Point3D<size_t> point) {
